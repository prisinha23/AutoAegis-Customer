...
public class MechanicAdapter extends RecyclerView.Adapter<MechanicAdapter.ViewHolder> {
    ...
    public MechanicAdapter(List<MechanicModel> mechanicModelList) {
        this.mechanicModelList = mechanicModelList;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        String resource=mechanicModelList.get(position).getProduct_image();
        String title=mechanicModelList.get(position).getProduct_title();
        String price=mechanicModelList.get(position).getProduct_price();
        holder.setData(resource,title,price);
    }

    @Override
    public int getItemCount() {
        return mechanicModelList.size();
    }
        ...
        private void setData(final String resource, final String title, final String price){
            ...
            add_to_cart.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    final String product_id=firebaseFirestore.collection("USERS").document(firebaseAuth.getCurrentUser().getUid()).collection("USER_CART").getId();
                    Map<Object,String>product_added_id=new HashMap<>();
                    product_added_id.put("product_image",resource);
                    product_added_id.put("product_id","mechanic_order_no"+value);
                    product_added_id.put("product_price",price);
                    product_added_id.put("product_title",title);
                    firebaseFirestore.collection("USERS").document(firebaseAuth.getCurrentUser().getUid()).collection("USER_CART").document("mechanic_order_no"+value).set(product_added_id).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
